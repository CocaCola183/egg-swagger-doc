...
function buildDefinition(source) {
  ...
  for (let object in source) {
    ...
    // 单独处理response为array的情况
    if (def.hasOwnProperty('type') && def.type == 'array' && !def.type.hasOwnProperty('type')) {
      if (!def.hasOwnProperty('itemType')) {
        throw new Error(`[egg-swagger-doc] ${path}: ${object}.itemType is necessary`);
      }
      if (!itemType.includes(def.itemType)) {
        let itemType = { $ref: `#/definitions/${def.itemType}` };
        def['items'] = itemType;
      } else {
        let itemType = { type: def.itemType };
        def['items'] = itemType;
      }
      delete def.itemType;
      target.type = 'array';
